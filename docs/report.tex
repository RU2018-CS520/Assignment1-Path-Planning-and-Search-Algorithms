	...
	\begin{enumerate}[resume]
		...
		\begin{enumerate}
			...
			\begin{enumerate}
				...
				\textbf{Patterns or trends:} 
				\begin{itemize}
					\item {A nearly "empty"(no walls) maze is usually hard for A* with Euclidean Distance Heuristic(A*E for short) in terms of time complexity.}
				\end{itemize}
				
				\textbf{Reasons:} 
				\begin{itemize}
					\item {A*E natrually tends to search all blocks, because Euclidean Distance is much smaller than actrually cost if this block is not in the last column and row.}
					\item {At each level, A*E will search diagnoal blocks, but find their expectation cost increased a lot. Then try to seach other blocks, which leads to these blocks' expectation cost also increased and even larger than diagnoal blocks.}
					\item {Therefore, A*E will regard every block as a worth searching block as long as the path do not turn up or left.}
					\item {Actually, if the whole maze have no northeast pathways or southwest pathways. A*E performs exactaly as same as BFS.}
				\end{itemize}
				
				\textbf{"The hardest" maze:} 
				\begin{itemize}
					\item {A totally "empty" maze whose number of blocks opened is exactly $size \times size = 16384$.}
					\item {The best score we have got is 14586, compared with at most 16384. However, it is really difficult to cancel out those walls by random. Therefore, it has been a nearly "hardest" maze.}
				\end{itemize}
				...
				\textbf{Patterns or trends:} 
				\begin{itemize}
					\item {The path is along the diagnoal line accurately and it seldom deviates from this direction.}
				\end{itemize}
				
				\textbf{Reasons:} 
				\begin{itemize}
					\item {This is because using the Euclidean distance as the distance function, the algorithm picks the path such that the Euclidean distance is minimized.}
					\item {To minimize the Eculidean distance, according to the Pythagorean theorem, the distance along the diagonal in a square-shaped maze is the optimal path. So the algorithm picks this path as the optimal path.}
				\end{itemize}
			...
			\begin{enumerate}
				...
				\textbf{Patterns or trends:} 
				\begin{itemize}
					\item {A nearly "empty"(no walls) maze. But there should be a narrow northeast or southwest pathway. And only this pathway leads to the Goal.}
				\end{itemize}
				
				\textbf{Reasons:} 
				\begin{itemize}
					\item {Once there is a counter-diagonal pathway near the Goal, the expectation cost of all blocks in the pathway will be increased.}
					\item {In this case, a totally empty outside will set every outside block with the same expectation cost, which is smaller than pathway blocks' expectation cost.}
				\end{itemize}
				
				\textbf{"The hardest" maze:} 
				\begin{itemize}
					\item {Compare to $size * size - WallCost =16377$, 14210 can still be improved. However, it is really difficult to cancel out those outside walls by random without destroying the pattern. Therefore, it has been a nearly "hardest" maze. See Figure \ref{fig:amb-pattern}.}
				\end{itemize}
				
				\begin{figure}
					\centering
					\includegraphics[width=0.3\textwidth]{../pics/amb/pattern.png}
					\caption{\label{fig:amb-pattern}}
				\end{figure}
