...
import solution
m = solution.buildUp()
count, path, depth = solution.DFS(checkFringe = True) #sometimes useful
```
DFS (today's special)
```
import solution
m = solution.buildUp()
count, path, depth = solution.DFS(quickGoal = True, randomWalk = True, checkFringe = True) #because it is coooooooooooool
```
